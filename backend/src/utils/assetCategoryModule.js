...
export async function resolveAssetCategoryModule(companyId, categoryType) {
  const raw = String(categoryType ?? "").trim();
  if (!raw) throw new AppError("Asset Category is required", 400, "VALIDATION_ERROR");
  ...
  if (md?.moduleKey) {
    const moduleKey = toModuleCode(md.moduleKey);
  ...
  throw new AppError("Unable to resolve Asset Category module key", 400, "VALIDATION_ERROR");
}
