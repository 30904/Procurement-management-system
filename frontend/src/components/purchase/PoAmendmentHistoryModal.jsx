...
import { getPurchaseOrderAmendmentHistoryRequest } from "../../services/api.js";
...
export default function PoAmendmentHistoryModal({ open, poId, poNo, onClose }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      try {
        const res = await getPurchaseOrderAmendmentHistoryRequest(poId);
      ...
      >
        ...
        >
          <span className="sc-modal-title">
            Amendment History — {poNo || data?.poNo || ""}
          </span>
                          ...
                          <tbody>
                            {row.changes.map((c, i) => (
                              <tr key={`${row.revisionNo}-${i}`}>
                                <td style={{ padding: "0.3rem", verticalAlign: "top" }}>{c.field}</td>
                                <td style={{ padding: "0.3rem", verticalAlign: "top" }}>{String(c.from ?? "—")}</td>
                                <td style={{ padding: "0.3rem", verticalAlign: "top" }}>{String(c.to ?? "—")}</td>
                              </tr>
                            ))}
                          </tbody>
                        </table>
