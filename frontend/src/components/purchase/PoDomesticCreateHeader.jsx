...
import styles from "./PoDomesticCreateHeader.module.css";

export default function PoDomesticCreateHeader({
  poNo,
  poNoLoading,
  poDate,
  onPoDateChange,
    ...
    <header className={styles.header}>
      <div className={styles.topRow}>
        <div className={styles.titleBlock}>
          <h2>{isEditMode ? "Edit domestic purchase order" : "New domestic purchase order"}</h2>
          <p>Card-based lines · full descriptions · GST on print</p>
        </div>
        <button
          type="button"
        ...
        >
          <UserCircle2 size={18} />
          {supplierName || "Choose domestic supplier"}
      ...
      <div className={styles.fieldsGrid}>
        ...
        <div className={styles.fieldMini}>
          ...
          />
          {validationErrors.poType ? (
            <div className={styles.fieldError}>{validationErrors.poType}</div>
          ) : null}
        </div>
      </div>
    </header>
