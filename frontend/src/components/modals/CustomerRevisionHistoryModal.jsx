...
function fieldLabel(field) {
  const labels = {
    customerNameOnTaxInvoice: "Customer Name on Tax Invoice",
    customerVendorCode: "Customer Vendor Code",
    customerNameLegalEntity: "Legal Entity Name",
    plantNameLocation: "Plant Name/Location",
    plantCodeId: "Plant Code",
    customerNickName: "Nick Name",
    isCustomerActive: "Status",
    customerCurrency: "Currency",
    creditLimit: "Credit Limit",
    customerPaymentTerms: "Payment Terms",
    customerFreightTerms: "Freight Terms",
    customerCIN: "PAN Card No.",
    gstClassification: "GST Classification",
    gstin: "GSTIN",
    customerBillingAddress: "Primary Address",
    customerShippingAddress: "Shipping Address",
    customerContactMatrix: "Contact Details",
  };
...
function buildHistoryRows(sourceRow) {
  ...
  const snapshot = {
    customerCode: sourceRow.customerCode,
    customerName: sourceRow.customerNameOnTaxInvoice,
    gstin: sourceRow.gstin,
    status: statusLabel(sourceRow.isCustomerActive),
  };
  ...
  for (const rev of sorted) {
    ...
    for (const ch of rev.changes || []) {
      if (ch.field === "customerNameOnTaxInvoice") snapshot.customerName = ch.from;
      if (ch.field === "gstin") snapshot.gstin = ch.from;
      if (ch.field === "isCustomerActive") snapshot.status = ch.from;
    }
  }
...
export default function CustomerRevisionHistoryModal({ open, sourceRow, onClose }) {
  const { modalRef, overlayStyle, modalStyle, handleHeaderMouseDown } = useModalDrag();
          ...
          <div className="rl-table-wrap">
            {pageRows.length === 0 ? (
              ...
              <table className="rl-table">
                <thead>
                  <tr>
                    <th style={{ width: "14%" }}>Customer Code</th>
