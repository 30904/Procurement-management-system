...
function buildHistoryRows(sourceRow) {
  ...
  const snapshot = {
    paymentTermsCode: sourceRow.paymentTermsCode,
    order: sourceRow.order ?? sourceRow.displayOrder,
  ...
  for (const rev of sorted) {
    ...
    for (const ch of rev.changes || []) {
      const key = ch.field === "order" ? "order" : ch.field;
      if (Object.prototype.hasOwnProperty.call(snapshot, key)) {
        snapshot[key] = ch.from;
      }
    }
  }
...
export default function PaymentTermsRevisionHistoryModal({ open, sourceRow, onClose }) {
  const { modalRef, overlayStyle, modalStyle, handleHeaderMouseDown } = useModalDrag();
          ...
          <div className="rl-table-wrap">
            {pageRows.length === 0 ? (
              ...
              <table className="rl-table">
                <thead>
                  <tr>
                    <th style={{ width: "18%" }}>Payment Terms Code</th>
                    <th style={{ width: "10%", textAlign: "center" }}>Order</th>
                    <th style={{ width: "36%" }}>Payment Terms Description</th>
