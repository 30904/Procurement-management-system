...
const FIELD_LABELS = {
  ...
  hsnCode: "HSN Code",
  gstRate: "GST Rate %",
  lifeExpectancyYears: "Life Expectancy (Year)",
  supplierName: "Vendor Name",
  manufacturerName: "Manufacturer Name",
  mpnModelNo: "MPN/Model No.",
  purchaseRateExGst: "Purchase Rate (Ex GST)",
  assetUniqueId: "Asset Unique ID",
  acquisitionDate: "Acquisition Date",
  capitalisationDate: "Capitalisation Date",
  inOperationDate: "In-Operation Date",
  manufacturingYear: "Manufacturing Year",
  ratedPowerKw: "Rated Power kW",
...
function buildHistoryRows(sourceRow) {
  ...
  const snapshot = { assetNo: sourceRow.assetNo, assetName: sourceRow.assetName, hsnCode: sourceRow.hsnCode, status: sourceRow.status };
  const rows = [];
  for (const rev of sorted) {
    rows.push({ id: `rev-${rev.revisionNo}`, ...snapshot, revNumber: rev.revisionNo, revision: rev });
    for (const ch of rev.changes || []) {
      if (ch.field === "assetName") snapshot.assetName = ch.from;
      if (ch.field === "hsnCode") snapshot.hsnCode = ch.from;
...
export default function AssetRevisionHistoryModal({ open, sourceRow, onClose }) {
  const { modalRef, overlayStyle, modalStyle, handleHeaderMouseDown } = useModalDrag();
      ...
      <div className="sc-modal-overlay" style={overlayStyle} onClick={(e) => e.target === e.currentTarget && onClose?.()}>
        <div ref={modalRef} className="sc-modal rl-modal" style={{ ...modalStyle, width: "72vw", maxWidth: "1120px", height: "68vh", minHeight: "460px" }}>
          ...
          <div className="rl-table-wrap">
            <table className="rl-table">
              <thead><tr><th>Asset No.</th><th>Asset Name</th><th>HSN Code</th><th style={{ textAlign: "center" }}>Status</th><th style={{ textAlign: "center" }}>Rev #</th><th style={{ textAlign: "center" }}>Rev Info</th></tr></thead>
              <tbody>
                {pageRows.length === 0 ? <tr><td colSpan={6} className="rl-empty">No revision history available.</td></tr> : pageRows.map((row) => (
                  <tr key={row.id}>
                    <td>{row.assetNo}</td><td>{row.assetName}</td><td>{row.hsnCode}</td>
