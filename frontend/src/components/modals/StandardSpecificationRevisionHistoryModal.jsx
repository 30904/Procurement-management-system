...
function fieldLabel(field) {
  const labels = {
    inspectionParameter: "Inspection/Test Parameter",
    uom: "UoM",
...
function buildHistoryRows(sourceRow) {
  ...
  const snapshot = {
    specId: sourceRow.specId,
    inspectionParameter: sourceRow.inspectionParameter,
    uom: sourceRow.uom,
    testStandard: sourceRow.testStandard,
    testMethod: sourceRow.testMethod,
  ...
  for (const rev of sorted) {
    ...
    for (const ch of rev.changes || []) {
      if (Object.prototype.hasOwnProperty.call(snapshot, ch.field)) {
        snapshot[ch.field] = ch.from;
      }
    }
  }

  if (sorted.length > 0) {
    rows.push({ id: "rev-base", ...snapshot, revNumber: 0, revision: null });
  } else {
    rows.push({
      id: "rev-current",
      ...snapshot,
                ...
                <tbody>
                  {changedFields.map((ch, idx) => (
                    <tr key={`${ch.field}-${idx}`}>
                      ...
                      <td>{displayDash(ch.from)}</td>
                      <td>{displayDash(ch.to)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
...
export default function StandardSpecificationRevisionHistoryModal({ open, sourceRow, onClose }) {
  const { modalRef, overlayStyle, modalStyle, handleHeaderMouseDown } = useModalDrag();
    ...
    <>
      ...
      >
        <div
          ...
          style={{ ...modalStyle, width: "78vw", maxWidth: "1180px", height: "68vh", minHeight: "460px" }}
        >
          <div className="sc-modal-bar" />
          <div className="sc-modal-header" onMouseDown={handleHeaderMouseDown} style={{ cursor: "grab" }}>
              ...
              <table className="rl-table">
                <thead>
                  <tr>
                    ...
                    <th style={{ width: "24%" }}>Inspection/Test Parameter</th>
                    <th style={{ width: "8%", textAlign: "center" }}>UoM</th>
                    <th style={{ width: "14%" }}>Test Standard</th>
                    <th style={{ width: "14%" }}>Test Method</th>
                    <th style={{ width: "10%", textAlign: "center" }}>Status</th>
                    <th style={{ width: "8%", textAlign: "center" }}>Rev #</th>
                    <th style={{ width: "8%", textAlign: "center" }}>Rev Info</th>
                  </tr>
                </thead>
                <tbody>
                  {pageRows.map((row) => {
                    ...
                    return (
                      <tr key={row.id}>
                        <td>{row.specId}</td>
                        <td>{row.inspectionParameter}</td>
                        <td style={{ textAlign: "center" }}>{row.uom}</td>
                        <td>{displayDash(row.testStandard)}</td>
