...
const FIELD_LABELS = {
  ...
  uom: "UoM",
  gstRegimeApplicability: "GST Regime Applicability",
  sacCode: "SAC Code",
  taxabilityType: "Taxability Type",
  gstRate: "GST Rate %",
  rcmApplicability: "RCM",
  itcAllowed: "ITC Allowed",
...
function buildHistoryRows(sourceRow) {
  ...
  const snapshot = { serviceId: sourceRow.serviceId, serviceName: sourceRow.serviceName, sacCode: sourceRow.sacCode, status: sourceRow.status };
  const rows = [];
  for (const rev of sorted) {
    rows.push({ id: `rev-${rev.revisionNo}`, ...snapshot, revNumber: rev.revisionNo, revision: rev });
    for (const ch of rev.changes || []) {
      if (ch.field === "serviceName") snapshot.serviceName = ch.from;
      ...
      <div ref={modalRef} className="sc-modal" style={{ ...modalStyle, width: "52vw", minWidth: "620px", maxWidth: "860px", maxHeight: "78vh" }}>
        ...
        <div className="sc-modal-body" style={{ padding: "2vh 1.5vw", gap: "1.2vh" }}>
          <div className="sc-field-grid sc-field-grid--2col">
            ...
            <div className="sc-field sc-field--full"><label className="sc-label sc-label-required">Reason for Revision</label><textarea className="sc-input" style={{ minHeight: "6.2vh", resize: "none" }} value={revision?.reason || ""} readOnly /></div>
          ...
          <table className="rl-table">
            ...
            <tbody>{(revision?.changes || []).map((ch, i) => <tr key={`${ch.field}-${i}`}><td>{FIELD_LABELS[ch.field] || ch.field}</td><td>{String(ch.from ?? "—")}</td><td>{String(ch.to ?? "—")}</td></tr>)}</tbody>
          </table>
        </div>
      </div>
    </div>,
...
export default function ServiceMasterR1RevisionHistoryModal({ open, sourceRow, onClose }) {
  const { modalRef, overlayStyle, modalStyle, handleHeaderMouseDown } = useModalDrag();
      ...
      <div className="sc-modal-overlay" style={overlayStyle} onClick={(e) => e.target === e.currentTarget && onClose?.()}>
        <div ref={modalRef} className="sc-modal rl-modal" style={{ ...modalStyle, width: "72vw", maxWidth: "1120px", height: "68vh", minHeight: "460px" }}>
          ...
          <div className="rl-table-wrap">
            <table className="rl-table">
              <thead><tr><th>Service ID</th><th>Service Name</th><th>SAC Code</th><th style={{ textAlign: "center" }}>Status</th><th style={{ textAlign: "center" }}>Rev #</th><th style={{ textAlign: "center" }}>Rev Info</th></tr></thead>
              <tbody>
                {pageRows.length === 0 ? <tr><td colSpan={6} className="rl-empty">No revision history available.</td></tr> : pageRows.map((row) => (
                  <tr key={row.id}>
                    <td>{row.serviceId}</td><td>{row.serviceName}</td><td>{row.sacCode}</td>
