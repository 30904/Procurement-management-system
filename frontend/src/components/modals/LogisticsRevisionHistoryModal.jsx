...
function fieldLabel(field) {
  const labels = {
    lspNameLegalEntity: "LSP Name",
    lspNickName: "LSP Nick Name",
    gstClassification: "GST Classification",
    gstin: "GSTIN",
    lspCIN: "PAN Card No.",
    rcmApplicability: "RCM Applicability",
    lspCurrency: "Currency",
    lspPaymentTerms: "Payment Terms",
    freightServiceType: "Type of Freight Service",
    isLspActive: "Status",
    lspAddress: "Address",
    lspContactMatrix: "Contact Details",
    lspBankDetails: "Bank Details",
    lspVehicleDetails: "Vehicle Details",
  };
...
function buildHistoryRows(sourceRow) {
  ...
  const snapshot = {
    lspCode: sourceRow.lspCode,
    lspName: sourceRow.lspNameLegalEntity,
    gstin: sourceRow.gstin,
    status: statusLabel(sourceRow.isLspActive),
  };
  const rows = [];
  for (const rev of sorted) {
    rows.push({ id: `rev-${rev.revisionNo}`, ...snapshot, revNumber: rev.revisionNo, revision: rev });
    for (const ch of rev.changes || []) {
      if (ch.field === "lspNameLegalEntity") snapshot.lspName = ch.from;
      if (ch.field === "gstin") snapshot.gstin = ch.from;
      if (ch.field === "isLspActive") snapshot.status = ch.from;
    }
  }
...
export default function LogisticsRevisionHistoryModal({ open, sourceRow, onClose }) {
  const { modalRef, overlayStyle, modalStyle, handleHeaderMouseDown } = useModalDrag();
      ...
      <div className="sc-modal-overlay" style={overlayStyle} onClick={(e) => e.target === e.currentTarget && onClose?.()}>
        <div ref={modalRef} className="sc-modal rl-modal" style={{ ...modalStyle, width: "72vw", maxWidth: "1120px", height: "68vh", minHeight: "460px" }}>
          ...
          <div className="rl-table-wrap">
            <table className="rl-table">
              <thead><tr><th>LSP Code</th><th>LSP Name</th><th>GSTIN</th><th style={{ textAlign: "center" }}>Status</th><th style={{ textAlign: "center" }}>Rev #</th><th style={{ textAlign: "center" }}>Rev Info</th></tr></thead>
              <tbody>
                {pageRows.length === 0 ? <tr><td colSpan={6} className="rl-empty">No revision history available.</td></tr> : pageRows.map((row) => (
                  <tr key={row.id}>
                    <td>{row.lspCode}</td><td>{row.lspName}</td><td>{row.gstin}</td>
