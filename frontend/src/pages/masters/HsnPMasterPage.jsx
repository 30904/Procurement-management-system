...
import HsnPMasterModal from "../../components/modals/HsnPMasterModal.jsx";
import HsnPRevisionModal from "../../components/modals/HsnPRevisionModal.jsx";
import HsnPRevisionHistoryModal from "../../components/modals/HsnPRevisionHistoryModal.jsx";
import ConfirmDialog from "../../components/common/ConfirmDialog.jsx";
import {
  listHsnPMasterRequest,
  createHsnPMasterRequest,
  updateHsnPMasterRequest,
  deleteHsnPMasterRequest,
...
const COLUMNS = [
  ...
  },
  {
    ...
    sortable: true,
    render: (v) => (
      <span className={pageStyles.revLink}>Rev {v ?? 0}</span>
    ),
  },
  {
    key: "status",
...
export default function HsnPMasterPage() {
  ...
  const fetchRows = useCallback(async () => {
    ...
    try {
      ...
      toast.error(err?.message || "Failed to load HSN/P records");
      setRows([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleDelete = async (row) => {
    ...
    setDeleting(true);
    try {
      await deleteHsnPMasterRequest(row._id || row.id);
      toast.success("HSN/P record deleted.");
      await fetchRows();
    } catch (err) {
  ...
  const handleSave = async (payload) => {
    try {
      if (editRow) {
        ...
        await updateHsnPMasterRequest(editRow._id || editRow.id, payload);
      } else {
        await createHsnPMasterRequest(payload);
      }
      await fetchRows();
    } catch (err) {
      const message =
        err?.data?.message || err?.message || "Failed to save HSN/P record";
  ...
  const handleRevisionSave = async (revisionInfo) => {
    ...
    try {
      ...
      toast.success("HSN/P record updated with revision details.");
      setRevisionModalOpen(false);
      setPendingEditPayload(null);
      setModalOpen(false);
      setEditRow(null);
  ...
  return (
    <div className={`erp-page ${styles.page}`}>
      ...
      <ConfirmDialog
        open={!!confirmTarget}
        title="Delete HSN/P Record"
        message={
          confirmTarget
            ? `Are you sure you want to delete HSN Code ${confirmTarget.hsnCode}? This action cannot be undone.`
            : ""
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
