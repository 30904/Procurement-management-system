import { useCallback, useEffect, useState } from "react";
import { Eye, FileClock, Pencil, Trash2 } from "lucide-react";
import MasterBreadcrumbToolbar from "../../components/masters/MasterBreadcrumbToolbar.jsx";
import { useFooter } from "../../context/FooterContext.jsx";
import { useToast } from "../../hooks/useToast.js";
import { getUserDisplayName } from "../../utils/authStorage.js";
import styles from "../../styles/page-toolbar.module.css";
import pageStyles from "./HsnPMasterPage.module.css";
import DataTable from "../../components/common/DataTable.jsx";
import SacPMasterModal from "../../components/modals/SacPMasterModal.jsx";
import SacPRevisionModal from "../../components/modals/SacPRevisionModal.jsx";
import SacPRevisionHistoryModal from "../../components/modals/SacPRevisionHistoryModal.jsx";
import ConfirmDialog from "../../components/common/ConfirmDialog.jsx";
import {
  listSacPMasterRequest,
  createSacPMasterRequest,
  updateSacPMasterRequest,
  deleteSacPMasterRequest,
} from "../../services/api.js";
import "../../styles/theme.css";
import "../../styles/global.css";
import "../../styles/subcomponents.css";
import "../../styles/erp-layout.css";

function formatRate(value) {
  if (value === null || value === undefined || value === "") return "—";
  const n = Number(value);
  if (Number.isNaN(n)) return String(value);
  return n.toFixed(2);
}

function normalizeRow(doc) {
  const id = doc._id != null ? String(doc._id) : String(doc.id);
  return {
    ...doc,
    _id: id,
    id,
    sacCode: doc.sacCode ?? "",
    description: doc.description ?? "",
    gstRate: doc.gstRate ?? 0,
    igstRate: doc.igstRate ?? 0,
    sgstRate: doc.sgstRate ?? 0,
    cgstRate: doc.cgstRate ?? 0,
    utgstRate: doc.utgstRate ?? 0,
    revNumber: doc.revNumber ?? 0,
    status: doc.status || "Active",
    governmentCategory: doc.mpbcdcTax?.governmentCategory ?? "",
  };
}

const COLUMNS = [
  {
    key: "sacCode",
    label: "SAC Code",
    width: "9%",
    align: "center",
    sortable: true,
    filterable: true,
  },
  {
    key: "description",
    label: "Description Of Services",
    width: "20%",
    align: "left",
    sortable: true,
    filterable: true,
  },
  {
    key: "gstRate",
    label: "GST %",
    width: "8%",
    align: "center",
    sortable: true,
    render: (v) => formatRate(v),
    filterable: true,
  },
  {
    key: "igstRate",
    label: "IGST %",
    width: "8%",
    align: "center",
    sortable: true,
    render: (v) => formatRate(v),
    filterable: true,
  },
  {
    key: "cgstRate",
    label: "CGST %",
    width: "8%",
    align: "center",
    sortable: true,
    render: (v) => formatRate(v),
    filterable: true,
  },
  {
    key: "sgstRate",
    label: "SGST %",
    width: "8%",
    align: "center",
    sortable: true,
    render: (v) => formatRate(v),
    filterable: true,
  },
  {
    key: "utgstRate",
    label: "UTGST %",
    width: "8%",
    align: "center",
    sortable: true,
    render: (v) => formatRate(v),
    filterable: true,
  },
  {
    key: "governmentCategory",
    label: "Gov. Category",
    width: "9%",
    align: "center",
    sortable: true,
    filterable: true,
  },
  {
    key: "revNumber",
    label: "Rev #",
    width: "7%",
    align: "center",
    sortable: true,
    render: (v) => <span className={pageStyles.revLink}>Rev {v ?? 0}</span>,
  },
  {
    key: "status",
    label: "Status",
    width: "9%",
    align: "center",
    filterable: true,
    type: "status",
  },
  { key: "action", label: "Action", width: "7%", align: "center" },
];

export default function SacPMasterPage() {
  const toast = useToast();
  const { setFooterContent } = useFooter();
  const [rows, setRows] = useState([]);
  const [loading, setLoading] = useState(true);
  const [modalOpen, setModalOpen] = useState(false);
  const [editRow, setEditRow] = useState(null);
  const [pendingEditPayload, setPendingEditPayload] = useState(null);
  const [revisionModalOpen, setRevisionModalOpen] = useState(false);
  const [revisionLogRow, setRevisionLogRow] = useState(null);
  const [confirmTarget, setConfirmTarget] = useState(null);
  const [deleting, setDeleting] = useState(false);

  const fetchRows = useCallback(async () => {
    setLoading(true);
    try {
      const res = await listSacPMasterRequest();
      const data = Array.isArray(res?.data) ? res.data : [];
      setRows(data.map(normalizeRow));
    } catch (err) {
      toast.error(err?.message || "Failed to load SAC/P records");
      setRows([]);
    } finally {
      setLoading(false);
    }
  }, [toast]);

  useEffect(() => {
    fetchRows();
  }, [fetchRows]);

  DataTable.useRecordCount(rows, setFooterContent);

  const handleDelete = async (row) => {
    if (!row) return;
    setDeleting(true);
    try {
      await deleteSacPMasterRequest(row._id || row.id);
      toast.success("SAC/P record deleted.");
      await fetchRows();
    } catch (err) {
      toast.error(err?.message || "Failed to delete record");
    } finally {
      setDeleting(false);
      setConfirmTarget(null);
    }
  };

  const handleSave = async (payload) => {
    try {
      if (editRow) {
        if (!payload?.revisionInfo) {
          setPendingEditPayload(payload);
          setRevisionModalOpen(true);
          return { deferred: true };
        }
        await updateSacPMasterRequest(editRow._id || editRow.id, payload);
      } else {
        await createSacPMasterRequest(payload);
      }
      await fetchRows();
    } catch (err) {
      const message =
        err?.data?.message || err?.message || "Failed to save SAC/P record";
      throw Object.assign(new Error(message), { data: err?.data });
    }
  };

  const handleModalClose = () => {
    setModalOpen(false);
    setEditRow(null);
    setPendingEditPayload(null);
    setRevisionModalOpen(false);
  };

  const handleRevisionSave = async (revisionInfo) => {
    if (!editRow || !pendingEditPayload) {
      setRevisionModalOpen(false);
      return;
    }
    try {
      await handleSave({ ...pendingEditPayload, revisionInfo });
      toast.success("SAC/P record updated with revision details.");
      setRevisionModalOpen(false);
      setPendingEditPayload(null);
      setModalOpen(false);
      setEditRow(null);
    } catch (err) {
      toast.error(err?.data?.message || err?.message || "Failed to save revision");
    }
  };

  const ACTION_OPTIONS = [
    {
      label: "View",
      icon: <Eye size={15} color="#0f3d91" strokeWidth={1.9} />,
      variant: "muted",
      onClick: (row) => {
        setEditRow(row);
        setModalOpen(true);
      },
    },
    {
      label: "Edit",
      icon: <Pencil size={15} color="#fb923c" strokeWidth={1.9} />,
      onClick: (row) => {
        setEditRow(row);
        setModalOpen(true);
      },
    },
    {
      label: "Revision Log",
      icon: <FileClock size={15} color="#22c55e" strokeWidth={1.9} />,
      disabled: (row) => Number(row?.revNumber || 0) === 0,
      onClick: (row) => setRevisionLogRow(row),
    },
    {
      label: "Delete",
      icon: <Trash2 size={15} color="#dc2626" strokeWidth={1.9} />,
      variant: "danger",
      onClick: (row) => setConfirmTarget(row),
    },
  ];

  return (
    <div className={`erp-page ${styles.page}`}>
      <MasterBreadcrumbToolbar
        defaultHubReturn="masters/purchase/gst-p"
        summaryTitle="SAC/P Summary"
      />

      <DataTable
        columns={COLUMNS}
        rows={rows}
        loading={loading}
        actions={ACTION_OPTIONS}
        searchPlaceholder="Search SAC code or description..."
        pageSize={10}
        onNew={() => {
          setEditRow(null);
          setModalOpen(true);
        }}
      />

      <ConfirmDialog
        open={!!confirmTarget}
        title="Delete SAC/P Record"
        message={
          confirmTarget
            ? `Are you sure you want to delete SAC Code ${confirmTarget.sacCode}? This action cannot be undone.`
            : ""
        }
        confirmLabel="Delete"
        cancelLabel="Cancel"
        variant="danger"
        loading={deleting}
        onConfirm={() => handleDelete(confirmTarget)}
        onCancel={() => (!deleting ? setConfirmTarget(null) : null)}
      />

      {modalOpen && (
        <SacPMasterModal
          initialData={editRow}
          onClose={handleModalClose}
          onSave={handleSave}
        />
      )}

      <SacPRevisionModal
        open={revisionModalOpen}
        revisionNo={Number(editRow?.revNumber || 0) + 1}
        defaultProposedBy={getUserDisplayName()}
        onClose={() => {
          setRevisionModalOpen(false);
          setPendingEditPayload(null);
        }}
        onSave={handleRevisionSave}
      />

      <SacPRevisionHistoryModal
        open={!!revisionLogRow}
        sourceRow={revisionLogRow}
        onClose={() => setRevisionLogRow(null)}
      />
    </div>
  );
}

