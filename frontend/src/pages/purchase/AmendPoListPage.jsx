...
import {
  ...
  listAmendablePurchaseOrdersRequest,
...
export default function AmendPoListPage() {
  ...
  const fetchRows = useCallback(async () => {
    setLoading(true);
    try {
      const res = await listAmendablePurchaseOrdersRequest();
  ...
  const openView = (row) => {
    ...
    navigate(appPath(`${LIST_PATH}/${id}`));
  };

  const openAmend = (row) => {
    const id = row.id || row._id;
    navigate(appPath(`${LIST_PATH}/${id}/edit`));
  };

  const openPrint = (row) => {
    const id = row.id || row._id;
    openAuthenticatedAppTab(appPath(PRINT_PATH(id)));
  };
  ...
  const handleApproveAmendment = async () => {
    ...
    setApproving(true);
    try {
      await approvePurchaseOrderAmendmentRequest(approveRow.id);
      toast.success(`Amendment approved for ${approveRow.poNo}.`);
      setApproveRow(null);
      await fetchRows();
    } catch (err) {
  ...
  const actions = useMemo(
    () => [
      {
        label: "Amend PO",
        icon: <Plus size={15} color="#e11d8f" strokeWidth={1.9} />,
    ...
    <div className={`erp-page ${styles.page} ${pageStyles.pageFill}`}>
      <header className={styles.toolbar}>
        ...
        <h1 className="erp-breadcrumb erp-breadcrumb--page-title">
          ...
          <span className="erp-breadcrumb-item">Amend PO</span>
        </h1>
      </header>
      ...
      <ConfirmDialog
        ...
        message={
          approveRow
            ? `Approve amendment for ${approveRow.poNo}? Revised values will replace the current approved PO.`
            : ""
        }
